...
export default function CreateCommunityForm() {
  ...
  // Controlled state for fields DexFetch can auto-fill
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Contract Address + DexScreener auto-fill */}
      <div className="rounded-xl border border-[#4ADE80]/20 bg-[#4ADE80]/[0.02] p-4 space-y-3">
        <div>
          ...
          <input
            ...
            placeholder="Paste contract address to auto-fill from DexScreener"
            maxLength={100}
            className="w-full rounded-lg border border-border-primary bg-bg-primary px-4 py-3 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-[#4ADE80] font-mono text-sm"
          />
        </div>
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full rounded-full bg-accent px-6 py-3 text-[15px] font-bold text-white transition-colors hover:bg-accent-hover disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {isPending ? (
