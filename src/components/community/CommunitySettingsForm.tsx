...
export default function CommunitySettingsForm({
  ...
  const [ca, setCa] = useState(community.contractAddress || "");
  ...
  function handleDexApply(data: Record<string, string | undefined>) {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* CA + DexScreener auto-fill */}
      <div className="rounded-xl border border-[#4ADE80]/20 bg-[#4ADE80]/[0.02] p-4 space-y-3">
        <div>
          <label htmlFor="settings-ca" className="mb-2 block text-sm font-medium text-text-primary">
            Contract Address (CA)
          </label>
          <input
            type="text"
            ...
            placeholder="Paste CA to auto-fill from DexScreener"
            maxLength={100}
            className="w-full rounded-lg border border-border-primary bg-bg-primary px-4 py-3 font-mono text-sm text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-[#4ADE80]"
          />
        </div>
        <DexFetch contractAddress={ca} onApply={handleDexApply} />
      </div>

      {/* Name */}
      <div>
        <label htmlFor="settings-name" className="mb-2 block text-sm font-medium text-text-primary">
          Name <span className="text-danger">*</span>
        </label>
        <input type="text" id="settings-name" name="name" required minLength={2} maxLength={50} value={name} onChange={(e) => setName(e.target.value)} placeholder="Community name" className="w-full rounded-lg border border-border-primary bg-bg-primary px-4 py-3 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
      </div>

      {/* Description */}
      <div>
        <label htmlFor="settings-description" className="mb-2 block text-sm font-medium text-text-primary">
          Description
        </label>
        <textarea id="settings-description" name="description" rows={3} maxLength={280} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="What is this community about?" className="w-full resize-none rounded-lg border border-border-primary bg-bg-primary px-4 py-3 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
      </div>

      {/* Social Links */}
      <div className="space-y-3">
        <label className="block text-sm font-medium text-text-primary">Links</label>
        ...
        <div className="flex items-center gap-3">
          ...
          <input type="text" name="twitter" value={twitter} onChange={(e) => setTwitter(e.target.value)} placeholder="X username or full URL" className="flex-1 rounded-lg border border-border-primary bg-bg-primary px-4 py-2.5 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input type="text" name="telegram" value={telegram} onChange={(e) => setTelegram(e.target.value)} placeholder="Telegram group or username" className="flex-1 rounded-lg border border-border-primary bg-bg-primary px-4 py-2.5 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input type="text" name="website" value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="Website URL" className="flex-1 rounded-lg border border-border-primary bg-bg-primary px-4 py-2.5 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <input type="text" name="dexscreener" value={dexscreener} onChange={(e) => setDexscreener(e.target.value)} placeholder="DexScreener URL" className="flex-1 rounded-lg border border-border-primary bg-bg-primary px-4 py-2.5 text-[15px] text-text-primary placeholder-text-secondary outline-none transition-colors focus:border-accent" />
        </div>
      </div>
